...
void HitBrick::addListener() {
	...
	touchListener->onTouchMoved = [](Touch* touch, Event* event)
	{
		...
		target->setPosition(Vec2(target->getPosition().x + touch->getDelta().x, target->getPosition().y));
...
void HitBrick::BrickGeneraetd() {
	...
	for (int i = 0; i < 3; i++) {
		...
		while (cw <= visibleSize.width) {
			...
			box->getPhysicsBody()->setRotationEnable(true);               
...
void HitBrick::pick_daoju(float delta)
{
	Vec2 pos = player->getPosition();

	for (int i=0; i<daojuVector.size(); i++)
	{

		auto daoju = daojuVector.at(i);

		if (player->getBoundingBox().intersectsRect(daoju->getBoundingBox()))
		{

			if (daoju->getTag() == 1) _scorevalue += 200;

			if (daoju->getTag() == 2) _scorevalue += 100;

			this->removeChild(daoju);

			daojuVector.eraseObject(daoju);
...
void HitBrick::Create_daoju(float delta)
{

	visibleSize = Director::getInstance()->getVisibleSize();

	int randX = rand() % (int)visibleSize.width + 20;

	if (_count % 2000 == 0)
	{
		auto daoju = Sprite::create("daoju1.png");

		daoju->setPosition(Vec2(randX, 20));

		daoju->setScale(0.5);

		this->addChild(daoju,20);

		daoju->setTag(1);

		daojuVector.pushBack(daoju);
	...
	else if (_count % 1000 == 0)
	{
		...
		auto daoju = Sprite::create("daoju2.png");
